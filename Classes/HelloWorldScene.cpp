...
void HelloWorld::update(float pDt)
{
	...
	if (IsHolding)
	{
		...
		this->addChild(dame.appearance,9);
		...
		if ( dame.direction == 0) dame.appearance->runAction(CCSequence::create(CCMoveBy::create(2, ccp(-150, 0)), CCCallFuncN::create(this, callfuncN_selector(HelloWorld::removeObject)), NULL));
		if ( dame.direction == 90) dame.appearance->runAction(CCSequence::create(CCMoveBy::create(2, ccp(0, 150)), CCCallFuncN::create(this, callfuncN_selector(HelloWorld::removeObject)), NULL));
		if ( dame.direction == 180) dame.appearance->runAction(CCSequence::create(CCMoveBy::create(2, ccp(150, 0)), CCCallFuncN::create(this, callfuncN_selector(HelloWorld::removeObject)), NULL));
		if ( dame.direction == 270) dame.appearance->runAction(CCSequence::create(CCMoveBy::create(2, ccp(0, -150)), CCCallFuncN::create(this, callfuncN_selector(HelloWorld::removeObject)), NULL));		
		Player_1.time = 50;
		}
	}
		...
		if (!check(pos)) 
			{
				...
				{
				if (type->compare("Brick") == 0) _background->removeTileAt( tileCoord );
